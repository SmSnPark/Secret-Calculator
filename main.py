#visual design of tkinter calculator taken from https://www.studytonight.com/tkinter/calculator-application-using-tkinter
...
def bt_equal():
    global expression
    if re.search('[a-zA-Z]', expression) or expression =="": #prevent any suspect or empty input
        input_text.set("N/A")
    else:
        try:
            result = str(eval(expression)) #potentially evil line, though I do scrub the expression . . . perhaps worth looking into more with advice from more experienced software safety experts
...
if __name__ == '__main__':
    ...
    if data == pswd:
        #this is the init password
        input_text.set("Add secrets here!")
    ...
    clear = Button(btns_frame, text = "C", fg = "black", width = 32, height = 3, bd = 0, bg = "white", cursor = "hand2", command = lambda: bt_clear()).grid(row = 0, column = 0, columnspan = 3, padx = 1, pady = 1)
    ...
    equals = Button(btns_frame, text = "=", fg = "black", width = 10, height = 3, bd = 0, bg = "white", cursor = "hand2", command = lambda: bt_equal()).grid(row = 4, column = 3, padx = 1, pady = 1)
